...
public final class MyWindowLevelTool extends PlugInTool implements ActionListener {
    ...
    void adjustWindowLevel(ImagePlus imp, double xDifference, double yDifference ) {
        ...
        int[] hi = imp.getStatistics().histogram;
        /*
        System.out.print("(" + hi.length + ") [");
        for(int i = 0; i < hi.length; i++) {
            System.out.print(hi[i] + ", ");
        }
        System.out.println("]");
        System.out.print(currentMax + ", " + currentMin + ": " + currentWindow + ", " + currentLevel);
        */

        //change
        double newWindow = currentWindow + xDifference;
        double newLevel = currentLevel + yDifference;
        // System.out.println(": " + newWindow + ", " + newLevel);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if ("Auto".equals(cmd)) {
            autoItemActionPerformed();
        }
        else if ("Reset".equals(cmd))
            resetItemActionPerformed();
        else maybeSetCt(cmd);
    }

    public void setupImage(ImagePlus imp, boolean fullSetup) {
        if( imp == null) return;
        if(fullSetup) {
            RGB = imp.getType() == ImagePlus.COLOR_RGB;
            currentMin = imp.getDisplayRangeMin();
    ...
    public void autoItemActionPerformed() {
    ...
    public void betterAutoItemActionPerformed(ImagePlus imp) {
        if( imp == null) return;

        int hmin, hmax;
        Calibration cal = imp.getCalibration();
        imp.setCalibration(null);
        ImageStatistics stat1 = imp.getStatistics();	// uncalibrated
        ...
        if( hmax >= hmin) {
            ...
            //adjustWindowLevel(imp, 0, 0);
        }
    }

    private void resetItemActionPerformed() {
        if( impLast == null || !impLast.isVisible()) return;
    ...
    public boolean isCTImage(ImagePlus img) {
