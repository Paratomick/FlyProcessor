 ...
 */
public class MyZProjector implements PlugIn {
    ...
    public MyZProjector(ImagePlus imp) {
        setImage(imp);
    }
    ...
    public static ImagePlus run(ImagePlus imp, String method, int startSlice, int stopSlice) {
        MyZProjector zp = new MyZProjector(imp);
    ...
    public void run(int startSlice, int stopSlice, int method) {
        imp = IJ.getImage();
        if (imp==null) {
            IJ.noImage();
            return;
        ...
        */
        ...
        if (isHyperstack)
            allTimeFrames = imp.getNFrames()>1&&imp.getNSlices()>1?false:false;
        doProjection(true);

        imp.unlock();
        IJ.register(MyZProjector.class);
    ...
    public void run(String arg) {
        ...
        if (this.imp == null) {
            ...
            IJ.error("Z Project", "Stack required");
        } else if (!this.imp.getProcessor().isInvertedLut() || IJ.showMessageWithCancel("ZProjection", "Stacks with inverter LUTs may not project correctly.\nTo create a standard LUT, invert the stack (Edit/Invert)\nand invert the LUT (Image/Lookup Tables/Invert LUT).")) {
            ...
            if (!gd.wasCanceled()) {
                if (this.imp.lock()) {
                    ...
                    Prefs.set("zproject.method", this.method);
                    ...
                    if (arg.equals("") && this.projImage != null) {
                        ...
                        this.projImage.show("ZProjector: " + IJ.d2s((double)(tstop - tstart) / 1000.0D, 2) + " seconds");
                    }

                    this.imp.unlock();
                    IJ.register(ZProjector.class);
                    if (Recorder.scriptMode()) {
                        String m = this.getMethodAsString();
